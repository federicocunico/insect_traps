...
def img_to_numpy(x):
    if hasattr(x, "numpy"):
        ...
        # Scale from [0, 1] to [0, 255] if values are in float range
...
def draw_detections(
    image, detections: List[object], out_path: Optional[str] = None, show: bool = False
) -> Image.Image:
...
def draw_gt_and_preds(
    ...
    for pi, p in enumerate(preds):
        ...
        draw.text((x1 + 2, text_y), text, fill="yellow", font=font)
